...
try:
    ...
    print 'not found in database: exiting'
    sys.exit()

fout=var+'_tavg_'+str(year)+'.nc'
entry=dict(model=exp,file=r['path']+'/tavg/'+fout,variable=var,start_date=start_date,end_date=end_date,name='annual')
R.close()
...
if os.path.exists('ttmp.nc'):
    os.system('rm -f ttmp.nc')
fnam=hashlib.md5(f).hexdigest()+'.nc'
cmd=cmd+' ttmp.nc'
os.system(cmd)
cmd='cdo -timselavg,12 ttmp.nc '+fnam
os.system(cmd)
